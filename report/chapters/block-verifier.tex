...
with a distinct contract set from the one used in universal contract verification, and some code to glue the repeated calls to \sail{execute}.
...
The first resource is \coq{ptstoinstr}, written in the notation \coq{"a 'i↦' id"}, where \coq{a} is an address and \coq{id} an union, which is either an instruction \coq{i} (of type \coq{ast}, union key \coq{I}) or an instruction argument \coq{d} (a word-size bitvector, union key \coq{D}). The meaning of the resource is that the location \coq{a} stores either \coq{d} or the binary encoding of \coq{i}.
...
\item in case of an instruction, a \coq{encodes_instr} chunk (notation: \coq{"w '≡' i"}) that records the fact that the concatenation of the two bytes is the binary encoding of the instruction;
...
\begin{listing}[htb]
  \begin{minted}{coq}
    Definition lemma_open_ptsto_instr : SepLemma open_ptsto_instr :=
      ...
      |}.
  \end{minted}
  \caption{Lemma to turn a \coq{ptstoinstr} into \coq{ptstomem} chunks. \coq{asn_ptsto_word a l h} is shorthand for \coq{a m↦ l ∗ a +' 1 m↦ h}.}
...
In the preconditions of the basic block contracts we will pass exactly the \coq{ptstomem} chunks that are needed for the execution of the object code. Consequently we have no need of assuming ownership of all (untrusted) memory locations with \coq{asn_accessible_addresses}, which was necessary in the universal contract as we were dealing with unknown programs.

The \coq{extract_accessible_ptsto} and \coq{return_accessible_ptsto} are still invoked in the \usail model---it is shared between the universal contract verification and the block verifier---but we can give them trivial (true) preconditions and postconditions to turn them into no-ops.

Incidentally, the same must be done for \coq{open_ptsto_instr} and \coq{close_ptsto_instr} in the universal contract verification, which doesn't use the \coq{ptstoinstr} resource.
...
In addition to the already discussed contract for \coq{decode}, we have to specify the other foreign functions, namely \coq{read_ram} and \coq{write_ram}. For them it's sufficient to reuse the contracts defined for the universal contract verification, see \cref{lst:read-write-foreign-contracts} for \coq{read_ram}.
...
We are ready to verify the bootcode's basic block. Due to limitations in the current implementation of the block verifier we will assume specific fixed values for the:
\begin{itemize}
...
% TODO some justification why this is not just testing but worse
% initial saved_ptr can be both 0 and the final
...
This block transfers the configuration structure address from \coq{ipe_struct_pointer_src} to \coq{saved_ptr_loc}, \ie a secured location that is supposed to be safe from later modification. It is executed only when the block shown in the previous section doesn't find a saved pointer.
...
The postcondition guarantees that the transfer has been performed (\coq{saved_ptr_loc} now contains the sample configuration address \coq{saved_ptr}) and that the PC is that of the next block, corresponding to the label \asm{check_struct}. Note that we treat these instruction as a basic block even if they include a jump, since the jump is only taken if the flag is not set.
...
% during verification a bug was found in the specification
% write_mpu_reg_byte was called first for the lsb
% which in the case of mpuipc0 contains the lock bit
% so writes to mpuipc0 that enabled locking would ignore the high byte
% inconsequential because high byte is reserved, but would have caused problems in case of extensions
% this was only found because of the precondition of write_mpu_reg_byte that says it must be unlocked
